              ...
              <div className="w-auto">
                ...
                <ul className="w-full flex flex-wrap justify-evenly">
                  {categories.map((val, key) => {
                      return (
                        <>
                          <li className="m-1 px-4 py-1 rounded-full bg-white hover:bg-[#2f9ff8] hover:text-white text-[#072d4b] shadow-md  cursor-pointer w-[200px] text-center inline-block" onClick={()=>{setCategory(val.name)}}>{val.name}</li>
                        </>
                      );
                    })}
                </ul>
