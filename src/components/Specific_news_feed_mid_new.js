...
export default function Specific_news_feed_mid_new(){
    ...
    //nested comment details
    const [nestedComment, setNestedComment] = useState("");
    const [nestedUserid, setNestedUserid] = useState("");
    const [nestedRealUserd, setNestedRealUserid] = useState("");
    ...
    const getCategories = () => {
      ...
      const postComment = () => {
        axios.post(`${API}/comments_neeraj`, {
          comment: comment,
          userid: userid,
          postid: postid,
        }).then((response) => {
...
//comment section
            ...
            <div className="">
                ...
                <input type="hidden"  value={1} onChange={(event) => {setPostid("1")}}/>
                <textarea
                name=""
                id=""
                cols={30}
                ...
                />
                <button className="px-8 py-[6px] bg-[#2f9ff8] text-white mt-3 rounded-sm" onClick={postComment}>Post Comment</button>
            </div>
